...
│   ├── run_rq2_experiment.py           ← RQ2: Ranking stability (attempt 3)
│   ├── run_rq3_experiment.py           ← RQ3: Score spread & formula checks
│   ├── visualize_rq1.py               ← RQ1 visualizations
│   ├── visualize_rq2.py               ← RQ2 visualizations
│   └── visualize_rq3.py               ← RQ3 visualizations
├── experiment_results/                 ← Generated by running experiments
│   ├── RQ1_results.json               ← Raw RQ1 data
│   ├── RQ2_results.json               ← Raw RQ2 data
│   ├── RQ3_results.json               ← Raw R3 data
│   ├── RQ1_self_retrieval_accuracy.md ← RQ1 methodology & results write-up
│   ├── RQ3_score_spread_validity.md   ← RQ3 technical methodology & results
│   ├── RQ3_score_spread_plain_language_explanation.md ← RQ3 plain-language summary
│   └── *.png                          ← Publication-ready figures for all RQs
...
| RQ | Question | Script | Metrics (high level) |
| --- | --- | --- | --- |
| **RQ1** | Does the system rank a song first when the user profile is derived from that song? (Accuracy) | `run_rq1_experiment.py` | HR@1, HR@3, HR@5, MRR with 95% bootstrap CI (over a random sample of valid query songs) |
| **RQ2** | Does the ranking stay stable when one favourite or avoid note is added/removed? (Robustness) | `run_rq2_experiment.py` | Kendall's τ per perturbation; overall mean/std across perturbations; baseline-level mean τ with 95% CI (bootstrap over baselines) |
| **RQ3** | Do scores spread out meaningfully, and does the scoring formula behave as specified? (Interpretability) | `run_rq3_experiment.py` | Variance & range of final_score; identity/regression checks for the formula; Pearson (final–cos/avoid) & Spearman (cos–fav overlap) correlations with 95% CI (Fisher-aggregated) |
